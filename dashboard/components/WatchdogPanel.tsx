/**
 * WatchdogPanel.tsx — S37 improved
 * Collapse/expand alerte, badge count pe titlu, threshold quick-view
 * Prop fullPage: afiseaza alerte fără max-h limit
 */
'use client';
...
async function postAction(path: string) {
  try { await fetch(`${API}${path}`, { method: 'POST' }); } catch { /* no-op */ }
}
    ...
    <div className="bg-gray-900 rounded-2xl p-5 w-full">
          ...
          {(status.halted_pairs?.length ?? 0) > 0 && (
            <div className="flex flex-col justify-center gap-1">
              {(status.halted_pairs as string[]).map(p => (
                <span key={p} className="text-xs bg-red-900 text-red-300 px-2 py-0.5 rounded-full">{p}</span>
              ))}
            </div>
          )}
        </div>
      ...
      <div className="flex flex-wrap gap-2 mb-5">
        <button onClick={enable}
          className="px-3 py-1.5 text-xs rounded-lg bg-green-900 hover:bg-green-800 text-green-200 transition-colors">
          Enable
        </button>
        ...
        <button onClick={silence}
          className="px-3 py-1.5 text-xs rounded-lg bg-yellow-900 hover:bg-yellow-800 text-yellow-200 transition-colors">
          Silence 1h
        </button>
      </div>
