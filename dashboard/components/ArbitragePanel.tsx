...
function TtlCountdown({ detectedAt, ttlSeconds }: { detectedAt: number; ttlSeconds: number }) {
  ...
  return (
    <span className={`font-mono text-xs tabular-nums ${remaining < 5 ? 'text-alert-danger font-bold animate-pulse' : 'text-text-muted'}`}>
...
export default function ArbitragePanel() {
  const { arbOpportunities } = useTradingStore()

  const sorted = [...arbOpportunities].sort((a, b) => b.spreadPct - a.spreadPct)

  const handleTrade = useCallback((opp: ArbOpportunity) => {
    // Notify — in live mode this would dispatch an order
    const msg = `▶ TRADE triggered: ${opp.pair} spread ${formatPercent(opp.spreadPct * 100, 3)}`
    console.info(msg)
    // Simple toast via DOM (no dep needed)
    const toast = document.createElement('div')
    toast.textContent = msg
    toast.className =
      'fixed bottom-6 right-6 z-50 rounded-md border border-neon-magenta bg-bg-panel px-4 py-2 font-mono text-sm text-neon-magenta shadow-lg'
                ...
                {sorted.map((opp) => (
                  <motion.tr
                    ...
                    className={`border-b border-bg-border font-mono ${
                      opp.spreadPct > 0.0003
                        ? 'border-l-2 border-l-neon-magenta bg-neon-magenta/10'
                        : ''
                    }`}
                  >
                    <td className="px-3 py-2 text-text-primary font-semibold">{opp.pair}</td>
                    <td className="px-3 py-2 tabular-nums text-text-primary">{formatPrice(opp.bybitPrice)}</td>
                    <td className="px-3 py-2 tabular-nums text-text-primary">{formatPrice(opp.binancePrice)}</td>
                    <td className="px-3 py-2 tabular-nums text-neon-green font-bold">
                      {formatPercent(opp.spreadPct * 100, 3)}
                    </td>
                    <td className="px-3 py-2">
                      <TtlCountdown detectedAt={opp.detectedAt} ttlSeconds={opp.ttlSeconds} />
                    </td>
                    <td className="px-3 py-2">
                      <button
