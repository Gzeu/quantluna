...
function UtcClock() {
  ...
  useEffect(() => {
    const tick = () => setTime(`${UTC_FMT.format(new Date())} UTC`)
    tick()
...
function WsOrb({ label, connected }: { label: string; connected: boolean }) {
  return (
    <span className="flex items-center gap-1 font-mono text-[10px] text-text-muted">
      <span className={`inline-block h-1.5 w-1.5 rounded-full ${connected ? 'bg-neon-green' : 'bg-alert-danger'}`} />
      {label}
    </span>
  )
}
...
export default function RegimeHeader() {
  const volatilityRegime      = useTradingStore((s) => s.volatilityRegime)
  const circuitBreakerOpen    = useTradingStore((s) => s.circuitBreakerOpen)
  const circuitBreakerCooldown = useTradingStore((s) => s.circuitBreakerCooldown)
  ...
  return (
    <header className="flex h-11 items-center justify-between border-b border-bg-border bg-bg-panel px-4 gap-3 shrink-0">
      ...
      <span className={`rounded-full border px-3 py-0.5 font-mono text-[10px] font-semibold ${REGIME_STYLES[volatilityRegime]}`}>
        {volatilityRegime}
      </span>
      {circuitBreakerOpen ? (
        <span className="flex items-center gap-1.5 rounded border border-alert-danger bg-alert-danger/20 px-2 py-0.5 font-mono text-[10px] text-alert-danger animate-pulse">
          CB OPEN {circuitBreakerCooldown > 0 && <span className="tabular-nums">{circuitBreakerCooldown}s</span>}
        </span>
      ) : (
      ...
      <button onClick={toggleMode}
        className={`rounded border font-mono text-[10px] px-2 py-0.5 transition-colors ${
          isLive ? 'border-alert-danger bg-alert-danger/20 text-alert-danger'
