...
interface CellProps {
  ...
  name?: string; change24h?: number
}
function CustomContent({ x = 0, y = 0, width = 0, height = 0, name = '', change24h = 0 }: CellProps) {
  if (width < 20 || height < 16) return null
  return (
    <g>
      <rect x={x} y={y} width={width} height={height} fill={interpolateColor(change24h)} stroke="#08080F" strokeWidth={1} />
      {width > 40 && height > 24 && (
        <>
          <text x={x + width / 2} y={y + height / 2 - 6} textAnchor="middle" fill="#E0E0F0" fontSize={10} fontFamily="monospace" fontWeight={600}>{name}</text>
          <text x={x + width / 2} y={y + height / 2 + 8} textAnchor="middle" fill={change24h >= 0 ? '#00FF88' : '#FF2244'} fontSize={9} fontFamily="monospace">{formatPercent(change24h)}</text>
        </>
      )}
    </g>
  )
...
interface TooltipEntry { payload: { name: string; price: number; change24h: number; volume24h: number; fundingRate: number } }
...
export default function MarketHeatmap() {
  ...
  const data = markets.map((m: MarketData) => ({
    ...
    change24h: m.change24h, price: m.price, volume24h: m.volume24h, fundingRate: m.fundingRate,
  }))
  ...
  return (
    <div className="flex flex-col h-full rounded-lg border border-bg-border bg-bg-panel overflow-hidden">
      <div className="flex items-center justify-between border-b border-bg-border px-4 py-2">
        ...
        <button onClick={() => setSizeMode((m) => m === 'change' ? 'volume' : 'change')}
      ...
      <div className="flex-1 min-h-0">
        <ResponsiveContainer width="100%" height="100%">
          <Treemap data={data} dataKey="size" aspectRatio={4 / 3} content={<CustomContent />}>
