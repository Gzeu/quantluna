...
Analog cu BacktestEngine dar ruleaza AutoSelector in loc de SignalGenerator.
...
class AutoSelectorRunner:
    ...
    def run(
        ...
        if regime_series is not None:            df["regime"]          = regime_series.values[:len(df)]
        elif "regime" not in df.columns:         df["regime"]          = "ranging"
        result_df = self.selector.generate_batch(
            df=df, funding_annual=funding_annual, regime_multiplier=regime_multiplier,
            zscore_col="zscore", spread_col="spread", half_life_col="half_life_hours",
            regime_col="regime", vol_rank_col="vol_rank",
        )
