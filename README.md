...
│   ├── spread_monitor.py         # Real-time spread health monitor  ← S18
...
│   ├── regime_filter.py          # Gate unificat regim  ← S18
│   ├── auto_selector.py          # Selectie automata perechi
│   └── trend_regime_detector.py  # Detectie trend vs mean-reversion
│
├── execution/
│   ├── order_manager.py          # Lifecycle comenzi multi-exchange  ← S17
│   ├── bybit_order_router.py     # Router Bybit futures
│   ├── binance_order_router.py   # Router Binance futures
│   ├── okx_order_router.py       # Router OKX futures  ← S16
│   ├── position_scanner.py       # Scan pozitii → MANAGED/ORPHAN  ← S17
│   ├── adoption_engine.py        # Decizie ADOPT/CLOSE_NOW/MONITOR  ← S17
│   ├── profit_optimizer.py       # TP/SL, break-even, trailing stop  ← S17
...
│
├── risk/
│   ├── circuit_breaker.py        # Circuit breaker auto-reset  ← S17
│   ├── kelly.py                  # Kelly criterion sizer
│   └── portfolio_risk.py         # Risk management portofoliu
│
├── notifications/
│   ├── notifier_bus.py           # Fan-out bus notificari  ← S18
│   ├── slack_notifier.py         # Notificari Slack  ← S17
...
│   ├── conftest.py
│   ├── test_sprint18.py          # SpreadMonitor + RegimeFilter + NotifierBus
│   ├── test_smoke_s18.py         # Smoke tests integrare S18
│   ├── test_sprint17.py          # OrderManager + CircuitBreaker + Slack
│   ├── test_smoke_s15_s17.py     # Smoke tests S15-S17
...
```bash
python main.py --mode paper --pair BTCUSDT/ETHUSDT
```

### Live trading

```bash
python main.py --mode live --venue bybit --pair BTCUSDT/ETHUSDT
```

### Backtest

```bash
python main.py --mode backtest --pair BTCUSDT/ETHUSDT --start 2024-01-01 --end 2024-12-31
```

### Walk-forward + Optuna

```bash
python main.py --mode walkforward --n-trials 200
```
...
Opreste intrarea in tranzactii cand oricare din 4 conditii nu este indeplinita:
...
if not cb.is_open:
    print(cb.status())  # {'is_open': False, 'cooldown_remaining_s': 3598, ...}
```
